...
export default function DehydratedRedOnionPage() {
  ...
  const redOnionProducts = [
    ...
    {
      name: "Dehydrated Red Onion Mince",
      size: "1 To 3 MM",
      image: "/images/products/red-onion-flakes.png"
    },
    {
    ...
    <main className="min-h-screen bg-gray-50/50 pt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-16">
        ...
        >
          ...
          <span>Back to Dehydrated Onion</span>
        </Link>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {redOnionProducts.map((item, itemIdx) => (
            <div
              key={itemIdx}
            ...
            >
              ...
              <div className="p-5 flex-grow flex flex-col justify-between border-t border-gray-100 bg-white">
                <h3 className="text-sm sm:text-base font-medium text-brand-primary group-hover:text-brand-primary/80 transition-colors duration-300 leading-snug tracking-tight">
                  {item.name}
                </h3>
                {item.size && (
                  <span className="text-xs sm:text-sm font-semibold text-gray-500 mt-2 block">
                    {item.size}
                  </span>
                )}
              </div>
