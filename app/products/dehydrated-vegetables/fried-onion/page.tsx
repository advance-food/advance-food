...
export default function FriedOnionPage() {
  const friedOnionProducts = [
    {
      name: "Dehydrate Fried Onion",
    ...
    <main className="min-h-screen bg-gray-50/50 pt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {friedOnionProducts.map((prod, idx) => (
            <Link
              key={idx}
              href={`/products/dehydrated-vegetables/fried-onion/${prod.slug}`}
            ...
            >
              ...
              <div className="p-5 flex-grow flex items-center justify-between border-t border-gray-100 bg-white">
                <span className="text-sm sm:text-base font-medium text-brand-primary group-hover:text-brand-primary/80 transition-colors duration-300 capitalize">
                  {prod.name}
                </span>
              </div>
            </Link>
